...
module conv2d (R:real) : layer with t = R.t
  ...
  --- Add zero padding around an 2D img
  ...
  let backward (act:[]t->[]t) (k:i32)
    ...
    let error' =
      ...
      then
        ...
        let w_flipped    = unsafe map (\i ->
                                       flatten ((map (\r ->
                                                      reverse (r[i:i+filter_sz])) w))) w_offsets
        let delta_padded = map (\delta' -> map (\x -> add_padding (k-1) x) delta') delta
        let delta_offsets= calc_img_offsets stride (x_m, x_n)
        let delta_matrix = map (\delta' -> im2col delta' (k,k) delta_offsets) delta_padded
