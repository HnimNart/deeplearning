...
module weight_initializer (R:real) : {
  ...
  val gen_random_array_2d_xavier_uni: (m:i32) -> (n:i32) -> i32 -> [n][m]t

  --- Using xavier norm initialize
  --- with mean = 0 and std = sqrt(2 / (fan_in + fan_out))
  val gen_random_array_2d_xavier_norm: (m: i32) -> (n: i32) -> i32 -> [n][m]t
  ...
  let gen_random_array_uni (d: i32) (dist) (seed: i32) : [d]t =
    map (\x -> gen_rand_uni x dist) (map (\x -> x + d + seed) (iota d))

  let gen_random_array_2d_xavier_uni (m: i32) (n: i32) (seed:i32) : [n][m]t =
  ...
  let gen_random_array_norm (d: i32) (seed: i32) (dist) : [d]t =
    map (\x -> gen_rand_norm x dist) (map (\x -> x + d + seed) (iota d))

  let gen_random_array_2d_xavier_norm (m: i32) (n: i32) (seed:i32) : [n][m]t =
