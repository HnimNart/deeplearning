...
module gradient_descent (R:real): optimizer with t = R.t with NN = NN R.t = {
  ...
  let feed_forward [p][q] (nn:NN) (input: [p][q]t) =
   ...
   let backprop_batch [m] (nn:NN) (input:[m][]t) (labels:[m][]t) (alpha: t) =
     ...
     let final_l_output = map (\x -> x[input_iter-n:input_iter]) input -- kan optimeres vaek
     let final_l_error  =
     ...
     let (grads_w, grads_b, cur_l_delta, _, input_iter, w_i, b_i) =
       loop (grads_w, grads_b, final_l_delta, l_iter, input_iter, w_i, b_i)
        while l_iter > 0 do
          ...
          let input_iter             = input_iter - cur_l_col
   ...
   let train_batch (nn:NN) (input:[][]t) (labels:[][]t) (alpha: t) =
     ...
     in {data = {w = new_w, b = new_b}, info = nn.info, nn_outputs = nn.nn_outputs }
